...
class DP_Exchange_Rate {
  ...
  protected static function fetch_exchange_rate_from_API( $fxpair ) {
    ...
    if ( is_wp_error( $resp ) ) {
      throw new Exception("Falha na conex&atilde;o com Exchange Rate API: " . $resp->get_error_message() );
    }

    $body = $resp['body'];
    if ( null === $body || 0 == strlen($body) ) {
      throw new Exception("Falha na conex&atilde;o com Exchange Rate API: " . $obj->error );
    }
